...
#modified 20180528 Hollie Putnam
#------------------------------------------------------------
rm(list=ls())
...
path<-"Data/20180529" #the location of all your titration files
massfile<-"20180529mass.csv" # name of your file with masses
...
pHCal<-read.csv('Data/pHCalibration.csv', header=T, sep=",", na.string="NA", as.is=T)  # read in the pH Calibration file
...
for(i in 1: length(file.names)) {
  ...
  #calculates the index of values between pH 3.0 and 3.5 
  ...
  d<-100*(-0.00000335*mean(Data$Temperature[mV], na.rm=T)^2-0.0001356*mean(Data$Temperature[mV], na.rm=T)+1.02613)/1000 #bottle changed 20180528
  #20180529 batch A3
  ...
  #s<-33.624 #batch 148 CRM
  s<-Mass$salinity
  #mass of sample in g: changed with every sample
  #mass<-Mass[name,1]
  mass<-Mass$mass
  #sample.index<-Mass[Mass$sample==name,1]# this is the order that the sample was run
  #-------------------------------------------------------------------
  #Calculate TA
